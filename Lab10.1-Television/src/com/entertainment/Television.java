...
public class Television {
    ...
    public void setBrand(String brand) throws InvalidBrandException {
        if( brand.equals("Samsung")  ||
            ...
            throw new InvalidBrandException("Invalid brand: " + brand + ", valid brands are Samsung, LG, Sony, Toshiba");
...
//            System.out.println("Invalid brand: " + brand + ", valid brands are Samsung, LG, Sony, Toshiba");
    ...
    */
    ...
    //NOTE: possibly throwing an UNCHECKED exception (IllegalArgumentException)
    //I do not need to "announce" this with 'throws' because it's unchecked but, we will to be a good citizen.
    public void setVolume (int volume) throws IllegalArgumentException{
        if (MIN_VOLUME <= volume && volume <= MAX_VOLUME) {
            ...
            throw new IllegalArgumentException("Invalid volume " + volume + ". Volume must be in " + MIN_VOLUME + " -" + MAX_VOLUME);
//                throw new IllegalArgumentException(String.format("Invalid volume: %s, volume must be in-between %s-%s\n", volume, MIN_VOLUME, MAX_VOLUME))
